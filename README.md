...
- [`log_summary.py`](tools/python/log_summary.py) — command-line log analysis utility. Accepts a log file, one or more optional case-insensitive keyword filters, and the number of patterns to show. Reports total and matching lines, file size, counts severity markers, surfaces high-severity and repeated message patterns, and shows the first and last matching lines.
